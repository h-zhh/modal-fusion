...
QUESTION_INSTRUCTION = (
    "I have an movie poster. "
    "Ask me questions about the content of this poster. "
...
MOVIE_INFO_TEMP = "Name: {}. Genre: {}. Plot: {}"
...
SUMMARY_INSTRUCTION = (
    "Now based on the information you get, generate a movie storyline in a paragraph."
    "Ignore the questions with answers no or not sure. "
    "Don't miss information. "
    "You are free to add information, but it has to be reasonable. \n"
    "Storyline: "
)
...
FIRST_QUESTION = "Describe this poster in detail."


VALID_CHATGPT_MODELS = ["gpt-3.5-turbo-1106"]
...
def call_gpt3(gpt3_prompt, max_tokens=40, model="text-davinci-003"):
    response = openai.Completion.create(
        model=model, prompt=gpt3_prompt, max_tokens=max_tokens
    )  # temperature=0.6,
...
def prepare_chatgpt_message(task_prompt, movie_info, questions, answers, sub_prompt):
    messages = [
        {"role": "system", "content": task_prompt},
        {"role": "system", "content": movie_info},
    ]
...
@retry(wait=wait_random_exponential(min=1, max=60), stop=stop_after_attempt(6))
def call_chatgpt(chatgpt_messages, max_tokens=40, model="gpt-3.5-turbo-1106"):
    client = OpenAI()
...
class AskQuestions:
    ...
    def __init__(
        self, img, blip2, model, movie_info, max_gpt_token=30, n_blip2_context=-1
    ):
        self.img = img
        self.blip2 = blip2
        self.model = model
...
def summarize_chat(questions, answers, model, movie_info, max_gpt_token=100):
    if model in VALID_GPT3_MODELS:
        summary_prompt = prepare_gpt_prompt(
            QUESTION_INSTRUCTION, movie_info, questions, answers, SUMMARY_INSTRUCTION
        )
...
def caption_image(
    blip2, image, model, movie_info, n_rounds=10, n_blip2_context=-1, print_mode="no"
):
    if model == "gpt3":
    ...
    results = {}
    chat = AskQuestions(
        image, blip2, model, movie_info, n_blip2_context=n_blip2_context
    )
    ...
    summary, summary_prompt, n_token_sum = summarize_chat(
        questions, answers, model=model, movie_info=movie_info
    )
...
def caption_images(
    ...
    for item in tqdm(dataset, disable=print_mode != "no"):
        img_id, image, name, intro, genre = item

        caption_path = os.path.join(
            save_path, "caption_result", "{}.yaml".format(img_id)
        )
        ...
        MOVIE_INFO = MOVIE_INFO_TEMP.format(name, genre, intro)
        ...
        with open(
            os.path.join(save_path, "output.csv"), "a", newline="", encoding="utf-8"
        ) as f:
            caption = info[blip2_tag]["ChatCaptioner"]["caption"]
            writer = csv.writer(f)
