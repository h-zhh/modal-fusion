...
class RandomSampledDataset:
    def __init__(self, datasets_root, dataset_name):
        ...
        self._ids = [
            file_name.split(".jpg")[0]
            for file_name in os.listdir(os.path.join(self.dataset_path, "img"))
        ]
            ...
            with open(ann_path, "r") as f:
                ...
                if isinstance(list(self.ann.keys())[0], int):
                    self.ann = {
                        ...
                        for image_id, captions in self.ann.items()
                    }
        else:
            self.ann = None
...
class PosterDataset(Dataset):
    def __init__(self, filepath, genre_sub=False, image_path="../../poster"):
        self.data_frame = pd.read_csv(filepath, index_col=0)
        self.genre_sub = genre_sub
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(os.path.join(self.image_path, f"{img_id}.jpg")).convert(
            "RGB"
        )
        name = str(self.data_frame.iloc[idx, 2])
        intro = str(self.data_frame.iloc[idx, -3])
        if self.genre_sub:
            intro = str(self.data_frame.iloc[idx, -4])
            genre = str(self.data_frame.iloc[idx, -1])
            print(
                "new genre: "
                + genre
                + "old genre: "
                + str(self.data_frame.iloc[idx, -3])
            )
        else:
            intro = str(self.data_frame.iloc[idx, -3])
            genre = str(self.data_frame.iloc[idx, -2])

        return img_id, image, name, intro, genre
...
def print_info(
    info, key="caption", variants=["BLIP2", "BLIP2+OurPrompt", "ChatCaptioner"]
):
